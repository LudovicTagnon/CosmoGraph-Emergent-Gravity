"""
Jackknife errors on P(k)/T(k) slopes over RA/DEC/z quantile chunks.
- Uses the SDSS slice already defined (RA 130-240, DEC -5-60, z 0.04-0.12).
- Builds k-NN graph (k configurable), weights = 1/d, strength = weighted degree.
- Leaves out each chunk (quantiles RA/DEC/z) -> recompute LCC, P(k), random catalog, T(k).
- Fits slopes on a fixed k-window.
Outputs JSON with mean±sigma jackknife for P_mass, P_topo, T_mass, T_topo.
"""
import json
...
OUT_JSON = Path(__file__).resolve().parent.parent / "outputs" / "jackknife_power_robust.json"


def load_sdss_slice(max_rows=12000):
...
def jackknife_from_dataframe(df, k_neighbors=10, grid=64, kmin=0.05, kmax=0.3, min_chunk=300):
    ...
    for idx, mask in enumerate(chunks):
        keep = ~mask
        df_sub = df[keep]
        if len(df_sub) < min_chunk:
            slopes.append({"P_mass": np.nan, "P_topo": np.nan, "T_mass": np.nan, "T_topo": np.nan, "n": len(df_sub)})
            continue
    ...
    result["chunk_sizes"] = [s.get("n", 0) for s in slopes]
    return result


def main():
    df = load_sdss_slice()
    res = jackknife_from_dataframe(df, k_neighbors=10, grid=64, kmin=0.05, kmax=0.3, min_chunk=300)
